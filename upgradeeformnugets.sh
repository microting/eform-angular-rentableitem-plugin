...
if (( "$GIT_STATUS" > 0 )); then
	git pull
	cd eFormAPI/Plugins/RentableItems.Pn/RentableItems.Pn
  CURRENT_NUMBER_OF_COMMITS=`git log --oneline | wc -l`

	PACKAGES=('Microting.eForm' 'Microting.eFormApi.BasePn' 'Microting.eFormBaseCustomerBase' 'Microting.eFormRentableItemBase')
	PROJECT_NAME='RentableItems.Pn.csproj'
	REPOSITORY='eform-angular-rentableitem-plugin'
