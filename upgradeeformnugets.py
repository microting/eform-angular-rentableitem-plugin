...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("eFormAPI/Plugins/RentableItems.Pn/RentableItems.Pn")
        ...
        process_repository('RentableItems.Pn.csproj', packages, 'eform-angular-rentableitem-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            os.chdir("../../../..")
            run_command(f"github_changelog_generator -u microting -p eform-angular-rentableitem-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
